 ...
 */
function getWorkspaceRoot(_ctx) {
    ...
    register(api) {
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                try {
                    ...
                    return textResult(`错误: ${err instanceof Error ? err.message : String(err)}`, { success: false, error: String(err) });
                }
            },
        });
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                try {
                    const p = params;
                    await agentGuideGenerator(p.workspace_root || join('.openclaw', 'workspaces', 'multi-agent-pipeline'), p.agent_name, p.instructions, p.append ?? false);
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                try {
                    const p = params;
                    const result = await pipelineStart(p.template_name, p.user_id, p.project_id, p.workspace_root || join('.openclaw', 'workspaces', 'multi-agent-pipeline'));
                    return jsonResult(result);
                }
                catch (err) {
                    return textResult(`错误: ${err instanceof Error ? err.message : String(err)}`, { success: false, error: String(err) });
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                try {
                    const p = params;
                    const result = await pipelineContinue(p.user_id, p.project_id, p.feedback, p.workspace_root || join('.openclaw', 'workspaces', 'multi-agent-pipeline'));
                    return jsonResult(result);
                }
                catch (err) {
                    return textResult(`错误: ${err instanceof Error ? err.message : String(err)}`, { success: false, error: String(err) });
