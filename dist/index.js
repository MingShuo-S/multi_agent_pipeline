    ...
    tools: (tool) => [
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const wsRoot = params.workspace_root || WS;
                    ...
                    const result = await pipelineRead({ agent_name: 'unknown', user_id: 'default-user', project_id: 'default-project', workspace_root: wsRoot }, params.slot_name, template);
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    ...
                    const pipelineState = await stateManager.load();
                    const template = await configManager.readTemplate(pipelineState.template_name);
                    await pipelineWriteSlot({ agent_name: 'unknown', user_id: 'default-user', project_id: 'default-project', workspace_root: wsRoot }, params.slot_name, params.content, template);
                    return '✅ 写入成功';
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const wsRoot = params.workspace_root || WS;
                    await pipelineAddRemark({ agent_name: 'unknown', user_id: 'default-user', project_id: 'default-project', workspace_root: wsRoot }, params.content);
                    return '✅ 批注已添加';
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const wsRoot = params.workspace_root || WS;
                    const result = await styleGetProfile({ agent_name: 'unknown', user_id: 'default-user', project_id: 'default-project', workspace_root: wsRoot });
                    return result;
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const wsRoot = params.workspace_root || WS;
                    await styleRecordFeedback({ agent_name: 'unknown', user_id: 'default-user', project_id: 'default-project', workspace_root: wsRoot }, params.preference_updates);
                    return '✅ 记忆已更新';
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const wsRoot = params.workspace_root || WS;
                    const result = await routeMessage({ agent_name: 'unknown', user_id: 'default-user', project_id: 'default-project', workspace_root: wsRoot }, params.target_agent, params.message);
                    return typeof result === 'string' ? result : result;
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const p = params;
                    await agentGuideGenerator(p.workspace_root || WS, p.agent_name, p.instructions, p.append ?? false);
                    return '✅ 指南已更新';
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const p = params;
                    const result = await pipelineStart(p.template_name, p.user_id, p.project_id, p.workspace_root || WS);
                    return result;
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
        ...
        tool({
            ...
            async execute(params, _config, _context) {
                try {
                    const p = params;
                    const result = await pipelineContinue(p.user_id, p.project_id, p.feedback, p.workspace_root || WS);
                    return result;
                }
                catch (err) {
                    return `错误: ${err instanceof Error ? err.message : String(err)}`;
