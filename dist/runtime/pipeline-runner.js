...
export class PipelineRunner {
    constructor(workspaceRoot, userId, projectId, templateName) {
    ...
    async run() {
        try {
            ...
            while (state.current_stage < template.stages.length && state.status === 'running') {
                ...
                // 构建 Prompt
                const promptBuilder = new PromptBuilder(this.workspaceRoot, this.userId, this.projectId);
    ...
    waitForCheckpointApproval(agentName, state, template, profile, promptBuilder) {
 ...
 */
export async function executeUntilCheckpoint(stateManager, template, context) {
    let state = await stateManager.load();
    const workspaceRoot = context.workspace_root;
    while (state.current_stage < template.stages.length) {
        const stage = template.stages[state.current_stage];
        const promptBuilder = new PromptBuilder(workspaceRoot, context.user_id, context.project_id);
        const profile = await new MemoryManager(workspaceRoot, context.user_id, stage.agent).getProfile();
        const prompt = await promptBuilder.buildPipelinePrompt(stage.agent, template, state, profile);
