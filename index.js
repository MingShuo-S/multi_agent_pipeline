...
export const tools = {
    ...
    workspace_config: {
        ...
        handler: async (params) => {
            try {
                const context = getContext();
                const result = await workspaceConfig(context.workspace_root, params.action, params);
                return { success: true, data: result };
            }
            catch (err) {
                return { success: false, error: String(err) };
    ...
    agent_guide_generator: {
        ...
        handler: async (params) => {
            try {
                const context = getContext();
                await agentGuideGenerator(context.workspace_root, params.agent_name, params.instructions, params.append || false);
                return { success: true };
            }
            catch (err) {
                return { success: false, error: String(err) };
    ...
    pipeline_start: {
        ...
        handler: async (params) => {
            try {
                ...
                const result = await pipelineStart(params.template_name, params.user_id, params.project_id, workspaceRoot);
                return { success: true, data: result };
            }
            catch (err) {
                return { success: false, error: String(err) };
    ...
    pipeline_continue: {
        ...
        handler: async (params) => {
            try {
                const workspaceRoot = getWorkspaceRoot();
                const result = await pipelineContinue(params.user_id, params.project_id, params.feedback, workspaceRoot);
                return { success: true, data: result };
            }
            catch (err) {
                return { success: false, error: String(err) };
