...
#
# 建议: 每次 pipeline_continue 写 kb.json 后触发，或 crontab 定时执行
...
$SharedDir = Join-Path $WorkspaceRoot "_shared"

if (-not (Test-Path $SharedDir)) {
  Write-Host "⚠ _shared/ 不存在: $SharedDir" -ForegroundColor Yellow
  exit 0
}
...
foreach ($uid in $UserDirs) {
  ...
  $KbPath = Join-Path $UserDir "kb.json"
  $AiPath = Join-Path $UserDir "kb.ai.md"

  if (-not (Test-Path $KbPath)) {
    Write-Host "  ⏭ $uid — kb.json 不存在" -ForegroundColor Gray
    continue
  }
  ...
  try {
    ...
    Write-Host "  ⚠ $uid — kb.json 解析失败: $_" -ForegroundColor Yellow
    continue
  }
  ...
  # 生成紧凑版 Markdown
  $lines = @("# KB 摘要（自动生成）", "> 源: kb.json | 生成: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')", "")
  ...
  Write-Host "  ✓ $uid — kb.ai.md ($($entries.Count) 条, $($groups.Count) 类)" -ForegroundColor Green
}
