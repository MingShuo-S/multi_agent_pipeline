...
SHARED_DIR="${WORKSPACE_ROOT}/_profiles"

if [ ! -d "$SHARED_DIR" ]; then
  echo "⚠ _profiles/ 不存在: $SHARED_DIR" >&2
  exit 0
fi

generate_memory_ai() {
  local user_dir="$1"
  local kb_path="${user_dir}/memory.json"
  local ai_path="${user_dir}/memory.ai.md"

  if [ ! -f "$kb_path" ]; then
    echo "  ⏭ $(basename "$user_dir") — memory.json 不存在" >&2
    return
  fi
...
lines = ['# MEMORY 摘要（自动生成）', '> 源: memory.json | 生成: $(date '+%Y-%m-%d %H:%M:%S')', '']
...
" > "$tmp"
  ...
  if $DRY_RUN; then
    echo "  [DRY] $(basename "$user_dir") — memory.ai.md ($(python3 -c "import json; print(len(json.load(open('$kb_path'))))") 条)"
    rm -f "$tmp"
  else
    mv "$tmp" "$ai_path"
    local count
    count=$(python3 -c "import json; print(len(json.load(open('$kb_path'))))")
    echo "  ✓ $(basename "$user_dir") — memory.ai.md ($count 条)"
  fi
}
...
if [ -n "$USER_ID" ]; then
  generate_memory_ai "${SHARED_DIR}/${USER_ID}"
else
  for d in "${SHARED_DIR}"/*/; do
    [ -d "$d" ] && generate_memory_ai "$d"
  done
fi
