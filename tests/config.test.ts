...
describe('config paths', () => {
  it('WORKSPACE_ROOT 不包含 templates 后缀', async () => {
    const { WORKSPACE_ROOT, SEED_TEMPLATES_DIR, PROFILES_DIR } = await import('../src/config.js');
    ...
    expect(PROFILES_DIR).toContain('_profiles');
  });

  it('PROFILES_DIR 是 WORKSPACE_ROOT 的子路径', async () => {
    const { WORKSPACE_ROOT, PROFILES_DIR } = await import('../src/config.js');
    expect(PROFILES_DIR).toContain(WORKSPACE_ROOT);
  });
